...
class ModernPatchAntennaDesigner:
    ...
    def _create_infinite_sphere_after(self, name="Infinite Sphere1"):
        """Cria a esfera infinita via módulo RadField (após o solve)."""
        try:
            ...
            self.log_message(f"Infinite sphere '{name}' created (theta -108..180, phi -180..180, 1deg).")
            return name
        except Exception as e:
    ...
    def _postprocess_after_solve(self):
        """Executa: criar p_i / ph_i, aplicar EditSources e criar esfera infinita."""
        try:
            try:
    ...
    def _get_gain_cut(self, frequency: float, cut: str, fixed_angle_deg: float):
        """Obtém dados de ganho em dB para um corte de theta ou phi em 'Infinite Sphere1'."""
        try:
            report_name = f"Gain_{cut}_cut_{fixed_angle_deg}deg"
            expression = "dB(GainTotal)"

            if cut.lower() == "theta":
                variations = {"Freq": f"{frequency}GHz", "Phi": f"{fixed_angle_deg}deg", "Theta": "All"}
                primary_sweep = "Theta"
            else:
                variations = {"Freq": f"{frequency}GHz", "Theta": f"{fixed_angle_deg}deg", "Phi": "All"}
                primary_sweep = "Phi"

            # ⚠️ Sem 'report_category' e usando 'setup' (não 'setup_name')
            ...
            sd = report.get_solution_data()
            if not sd:
                return None, None

            # sd.primary_sweep_values já costuma vir como floats; normalizamos mesmo assim
            angles = np.array([float(x) for x in sd.primary_sweep_values], dtype=float)

            # data_real() pode retornar [list] ou ndarray; tratamos genericamente
            data = sd.data_real()
            if isinstance(data, (list, tuple)) and len(data) > 0 and hasattr(data[0], "__len__"):
                gains = np.array(data[0], dtype=float)
            else:
                gains = np.array(data, dtype=float)

            if angles.size and angles.size == gains.size:
                return angles, gains
