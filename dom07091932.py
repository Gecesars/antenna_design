...
class ModernPatchAntennaDesigner:
    def __init__(self):
        ...
        self.created_ports: List[str] = []  # nomes criados, p.ex. ['P1_Lumped', ...]
        self.simulation_data = None         # (freq, s11_dB)
        self.last_s11_analysis = None       # dict com f_res, Z, etc.
        ...
        self.params = {
            ...
            "sweep_step": 0.02              # GHz (em Discrete)
        }
    ...
    def setup_gui(self):
        ...
        self.status_label = ctk.CTkLabel(
            status, text="Ready to calculate parameters", font=ctk.CTkFont(weight="bold")
        )
    ...
    def create_section(self, parent, title, row, column, padx=10, pady=10):
        ...
        ).grid(row=0, column=0, sticky="w", padx=15, pady=(10, 6))
    ...
    def setup_results_tab(self):
        ...
        ctk.CTkLabel(main, text="S11 Analysis & Tuning", font=ctk.CTkFont(size=18, weight="bold")).grid(row=0, column=0, pady=10)

        graph = ctk.CTkFrame(main); graph.grid(row=1, column=0, sticky="nsew", padx=10, pady=10)
        graph.grid_columnconfigure(0, weight=1); graph.grid_rowconfigure(0, weight=1)
        ...
        ctrl = ctk.CTkFrame(main); ctrl.grid(row=2, column=0, pady=8)
        ...
        ctk.CTkButton(ctrl, text="Export CSV", command=self.export_csv,
                      fg_color="#6A5ACD", hover_color="#7B68EE").pack(side="left", padx=8)
    ...
    def export_csv(self):
        try:
            if self.last_s11_analysis:
                ...
                np.savetxt("simulation_results.csv",
                           np.column_stack((f, s11)),
    ...
    def _create_coax_feed_lumped(self, ground, substrate, x_feed: float, y_feed: float, name_prefix: str):
        try:
            ...
            shield_inner_void = self.hfss.modeler.create_cylinder(
                orientation="Z", origin=[x_feed, y_feed, -Lp_val],
                radius=b_val, height=Lp_val, name=f"{name_prefix}_ShieldInnerVoid", material="vacuum")
    ...
    def _postprocess_after_solve(self):
        try:
            try:
                ...
                exs = []
            if not exs and self.created_ports:
                exs = [f"{p}:1" for p in self.created_ports]
            if not exs:
    ...
    # ------------- Safe Solution Fetchers -------------
    def _fetch_solution(self, expression: str, setup_candidates: Optional[List[str]] = None, **kwargs):
        """Usa post.get_solution_data com fallbacks para Sweep1 e LastAdaptive."""
        ...
        for setup in setup_candidates:
            try:
                sd = self.hfss.post.get_solution_data(
                    expressions=[expression], setup_sweep_name=setup, **kwargs
                )
    ...
    def _shape_series(self, data_obj, npoints: int) -> np.ndarray:
        ...
        if npoints > 0 and arr.size not in (1, npoints):
            # dados inconsistentes: retorna vazio
            return np.array([], dtype=float)
        return arr
    ...
    def _list_excitations(self) -> List[str]:
        ...
        # ordenar por índice numérico se possível
        def keyfn(s):
            import re
    ...
    def _get_gain_cut(self, frequency: float, cut: str, fixed_angle_deg: float):
        try:
            ...
            if not sd:
                # fallback via report (em algumas versões, far_field retorna Report com get_solution_data)
                try:
                    rpt = self.hfss.post.reports_by_category.far_field(
                        expressions=[expr], context="Infinite Sphere1",
                        primary_sweep_variable=prim, setup_name="Setup1 : Sweep1",
                        variations=variations, name=f"Gain_{cut}_cut_tmp"
                    )
                    sd = rpt.get_solution_data()
                except Exception:
                    sd = None
            ...
            gains_raw = sd.data_real()
            gains = self._shape_series(gains_raw, ang.size)
    ...
    def run_simulation(self):
        try:
            ...
            substrate = self.hfss.modeler.create_box(origin=["-subW/2", "-subL/2", 0],
                                                     sizes=["subW", "subL", "h_sub"], name="Substrate", material=sub_name)
            self.log_message("Creating ground plane")
            ground = self.hfss.modeler.create_rectangle(orientation="XY", origin=["-subW/2", "-subL/2", 0],
                                                        sizes=["subW", "subL"], name="Ground", material="copper")
            ...
            for r in range(rows):
                for c in range(cols):
                    ...
                    patch = self.hfss.modeler.create_rectangle(orientation="XY", origin=origin,
                                                               sizes=["patchW", "patchL"], name=patch_name, material="copper")
                    ...
                    pad = self.hfss.modeler.create_circle(orientation="XY", origin=[x_feed, y_feed, "h_sub"],
                                                          radius="a", name=f"{patch_name}_Pad", material="copper")
                    try: self.hfss.modeler.unite([patch, pad])
                    except Exception: pass

                    self._create_coax_feed_lumped(ground=ground, substrate=substrate,
            ...
            setup.props["Frequency"] = f"{self.params['frequency']}GHz"; setup.props["MaxDeltaS"] = 0.02
    ...
    def _get_s11_curves(self):
        """Retorna (f, s11_dB, reS, imS) com tentativas por nome e por índice."""
        ...
        for expr_tpl in [f"( {port_name},{port_name} )", f"({port_name},{port_name})"]:
            # dB
            sd_db = self._fetch_solution(f"dB(S{expr_tpl})")
            if sd_db and hasattr(sd_db, "primary_sweep_values"):
                ...
                if y_db.size == f.size and f.size > 0:
                    # Real/Imag para impedância
                    sd_re = self._fetch_solution(f"re(S{expr_tpl})")
                    sd_im = self._fetch_solution(f"im(S{expr_tpl})")
        ...
        # 2) Por índice
        idx = 1
        if self.created_ports and port_name in self.created_ports:
            idx = self.created_ports.index(port_name) + 1
        expr_tpl = f"({idx},{idx})"
        sd_db = self._fetch_solution(f"dB(S{expr_tpl})")
        if not sd_db:
            return None
    ...
    def analyze_and_mark_s11(self):
        """Analisa S11, marca o mínimo, calcula Z e plota cortes de ganho."""
        try:
            ...
            # Plot S11
            self.ax_s11.plot(f, s11_db, linewidth=2, label="S11")
            ...
            if reS is not None and imS is not None and reS.size == imS.size == f.size:
                ...
                try:
                    Z = Z0 * (1 + S) / (1 - S)
                    R = float(np.real(Z)); X = float(np.imag(Z))
                except ZeroDivisionError:
                    pass

            # Guardar para export/correção
            self.last_s11_analysis = {"f": f, "s11_db": s11_db, "f_res": f_res,
                                      "s11_min_db": s11_min_db, "R": R, "X": X}
            ...
            if th is not None and gth is not None:
                self.ax_th.plot(th, gth, linewidth=2); self.ax_th.set_xlabel("Theta (deg)")
                self.ax_th.set_ylabel("Gain (dB)"); self.ax_th.set_title("Radiation Pattern - Theta cut (Phi=0°)")
            ...
            if ph is not None and gph is not None:
                self.ax_ph.plot(ph, gph, linewidth=2); self.ax_ph.set_xlabel("Phi (deg)")
                self.ax_ph.set_ylabel("Gain (dB)"); self.ax_ph.set_title("Radiation Pattern - Phi cut (Theta=90°)")
            ...
            if R is not None and X is not None:
                ...
                self.result_label.configure(text=f"Min @ {f_res:.4g} GHz, S11={s11_min_db:.2f} dB (Z unavailable)")
            self.log_message("S11 analyzed and plotted.")
        except Exception as e:
            self.log_message(f"Analyze S11 error: {e}\nTraceback: {traceback.format_exc()}")

    def apply_frequency_correction(self):
        """Escala dimensões pelo fator λ_res/λ_target (equivalente a f_res/f_target)."""
        try:
            ...
            if f_res <= 0 or f_target <= 0:
                ...
                return
            scale = f_res / f_target  # se res < target → scale<1 (diminuir dimensões)
            # aplica em dimensões principais
            self.calculated_params["patch_length"] *= scale
            self.calculated_params["patch_width"]  *= scale
            self.calculated_params["spacing"]      *= scale
            self.calculate_substrate_size()

            # Atualiza variáveis no HFSS se projeto estiver aberto
            try:
            ...
            sense = "decreased" if scale < 1 else ("increased" if scale > 1 else "unchanged")
            self.log_message(f"Applied frequency correction: scale={scale:.6f} → dimensions {sense}.")
            self.result_label.configure(text=f"Applied correction (scale={scale:.6f}). Re-run simulation to verify.")
        except Exception as e:
            self.log_message(f"Apply correction error: {e}\nTraceback: {traceback.format_exc()}")

    # ------------- Plot wrapper (chama Analyze S11 e cortes) -------------
    def plot_results(self):
        """Compat: apenas delega para analyze_and_mark_s11 após solve."""
