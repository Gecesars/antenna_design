...
class ModernPatchAntennaDesigner:
    def __init__(self):
        ...
        # -------- Pesos de excitação (pós-processamento) --------
    ...
    def _maximize_with_taskbar(self):
        """Maximiza a janela mantendo a barra de tarefas visível (sem fullscreen)."""
        try:
            # garanta que não está em fullscreen
            self.window.attributes("-fullscreen", False)
        except Exception:
            pass
        # tenta o estado 'zoomed' (Windows)
        try:
            self.window.state("zoomed")
            return
        except Exception:
            pass
        # tenta atributo -zoomed (alguns WMs no Linux)
        try:
            self.window.attributes("-zoomed", True)
            return
        except Exception:
            pass
        # fallback: ocupar a tela inteira pela geometria (a barra pode sobrepor)
        try:
    ...
    def setup_gui(self):
        ...
        # abre grande mas manteremos a taskbar
        self.window.geometry("1400x900")
        self.window.protocol("WM_DELETE_WINDOW", self.on_closing)
        # maximiza sem fullscreen (barra de tarefas visível)
    ...
    def update_ports_table(self, n_ports: int):
        """Reconstrói a tabela de entradas (amp/phase) para n portas."""
        # preserva valores já inseridos
        self._sync_port_params_from_ui()

        # ajusta tamanho do vetor de modelo
    ...
    def _reset_port_weights(self):
        """Broadside: todas as potências = 1, fases = 0."""
    ...
    def _sync_port_params_from_ui(self):
        """Lê a tabela (se existir) e atualiza self.port_params."""
    ...
    def apply_postproc_variables(self):
        """Cria/atualiza variáveis de pós-processamento no HFSS: Pow_Pi e Phi_Pi (deg)."""
        try:
            self._sync_port_params_from_ui()
            ...
            for i in range(n):
                ...
                # potência relativa (linear) e fase em graus
                self.hfss[f"Pow_P{idx}"] = str(amp)
                self.hfss[f"Phi_P{idx}"] = f"{ph}deg"
            self.log_message(f"Applied post-processing vars for {n} ports (Pow_Pi, Phi_Pi).")
    ...
    def setup_results_tab(self):
        ...
        if ctk.get_appearance_mode() == "Dark":
            ...
            for s in ['bottom', 'top', 'right', 'left']:
                self.ax.spines[s].set_color('white')
    ...
    def calculate_parameters(self):
        ...
        try:
            ...
            # >>> Atualiza a tabela de portas conforme novo nº de elementos
            self.update_ports_table(self.calculated_params["num_patches"])
    ...
    # ------------- Simulação (sem alterações funcionais) -------------
    #  >>>>>>  A PARTIR DAQUI, TODO O CÓDIGO É O MESMO QUE VOCÊ JÁ TINHA  <<<<<<
    #  Mantive intacto para cumprir o pedido de "modificar apenas a GUI".
    #  (Funções: _set_design_variables, _create_coax_feed_lumped, start/stop_simulation,
    #   run_simulation, plot_results, cleanup, on_closing, save/load/update, run)
    #  -----------------------------------------------------------------

    # ------- As funções abaixo são copiadas sem mudanças funcionais -------
    ...
    def run_simulation(self):
        # (idêntico ao seu – sem mudanças na lógica de modelagem/solve)
        try:
        ...
        try:
            ...
            if sol:
                ...
                if y.size == freqs.size:
                    ...
                    self.ax.plot(freqs, y, label=expr, linewidth=2)
                    ...
                    self.ax.text(cf, self.ax.get_ylim()[1]*0.9, f'{cf} GHz')
                    self.canvas.draw()
                    self.log_message("Results plotted successfully")
                    return

            self.log_message("No data from named-port expression, trying S(1,1)")
            ...
            if sol:
                ...
                if y.size == freqs.size:
                    self.simulation_data = np.column_stack((freqs, y))
                    self.ax.plot(freqs, y, label="dB(S(1,1))", linewidth=2)
                    self.ax.axhline(y=-10, linestyle='--', alpha=0.7, label='-10 dB')
                    self.ax.set_xlabel("Frequency (GHz)")
                    self.ax.set_ylabel("S-Parameter (dB)")
                    self.ax.set_title("S11 - Coax-fed Patch Array (Lumped Ports)")
                    ...
                    self.ax.grid(True)
                    self.canvas.draw()
                    self.log_message("Results plotted successfully (fallback)")
                else:
    ...
    def update_interface_from_params(self):
        try:
            ...
            # atualiza tabela de portas conforme nº de elementos e preenche valores
            self.update_ports_table(self.calculated_params["num_patches"])
            # insere os valores carregados
            for i, (e_amp, e_ph) in enumerate(self.port_entries):
                e_amp.delete(0, "end")
                e_ph.delete(0, "end")
                e_amp.insert(0, str(self.port_params[i]["amp"]))
