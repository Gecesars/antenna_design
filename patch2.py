...
- Porta lumped criada corretamente entre o patch e o plano de terra.
- Setup nomeado (OpSetup) e sweep FAST (OpFast, 101 pts) com faixa ampliada em +30%.
- Execução explícita do setup correto e relatório S11 referenciando "OpSetup : OpFast".
- Liberação garantida do AEDT ao fechar a aplicação.

Correções aplicadas:
1. Posicionamento correto das portas lumped entre o patch e o plano de terra
2. Uso adequado das funções do PyAEDT para criação de geometria e portas
3. Correção do cálculo da impedância do transformador λ/4
4. Melhoria no gerenciamento de variáveis e parâmetros
"""
...
from ansys.aedt.core.modules.solutions import FrequentialSweep
...
def build_array_project(
    ...
    gnd = hfss.modeler.create_box(
        position=[f"-sx/2", f"-sy/2", "0"],
        dimensions_list=["sx", "sy", "tCu"],
        name="GND", 
        material="pec"
    )
    ...
    sub = hfss.modeler.create_box(
        position=[f"-sx/2", f"-sy/2", "tCu"],
        dimensions_list=["sx", "sy", "h"],
        name="SUB", 
        material="FR4_epoxy"
    )
    ...
    for ix in range(nx):
        for iy in range(ny):
            ...
            patch = hfss.modeler.create_box(
                position=[f"{cx_s}-Wp/2", f"{cy_s}-Lp/2", z_top],
                dimensions_list=["Wp", "Lp", "tCu"],
            ...
            line = hfss.modeler.create_box(
                position=[f"{cx_s}-Wt/2", line_y0, z_top],
                dimensions_list=["Wt", "Lq", "tCu"],
                name=f"TPLINE_{ix+1}_{iy+1}", 
                material="pec"
            )
            
            pad = hfss.modeler.create_box(
                position=[f"{cx_s}-Wt/2", feed_y0, z_top],
                dimensions_list=["Wt", "Pad", "tCu"],
                name=f"FEEDPAD_{ix+1}_{iy+1}", 
                material="pec"
            )

            # -------- Porta lumped entre o feed pad e o GND --------
            # Encontra a face inferior do feed pad
            pad_faces = pad.faces
            bottom_face = None
            for face in pad_faces:
                if abs(face.center[2] - (h_mm + COPPER_T/1000)) < 1e-6:  # z ≈ h_mm + tCu
                    bottom_face = face
                    break
            
            # Encontra a face superior do GND
            gnd_faces = gnd.faces
            top_gnd_face = None
            for face in gnd_faces:
                if abs(face.center[2] - COPPER_T/2000) < 1e-6:  # z ≈ tCu/2
                    top_gnd_face = face
                    break
            
            # Cria a porta lumped entre o feed pad e o GND
            if bottom_face and top_gnd_face:
                pname = f"P{len(port_names)+1}"
                
                # Cria a porta usando as faces identificadas
                port = hfss.lumped_port(
                    signal=bottom_face,
                    reference=top_gnd_face,
                    impedance=Z0_PORT, 
                    renormalize=True, 
                    name=pname
                )
    ...
    # ---- Região aberta ----
    hfss.create_open_region(frequency="f0")
...
class App(ctk.CTk):
    def __init__(self):
        ...
        self._log("[Dica] Porta lumped criada corretamente entre o feed pad e o GND.")
