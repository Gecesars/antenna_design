...
class ModernPatchAntennaDesigner:
    ...
    def setup_impedance_tab(self, parent):
        ...
        self.ax_z.legend(["Real", "Imaginary"], loc="upper right")
    ...
    def setup_radiation_tab(self, parent):
        ...
        self.fig_rad = plt.figure(figsize=(10, 8))
        face = '#2B2B2B' if ctk.get_appearance_mode() == "Dark" else '#FFFFFF'
        ...
        self.ax_3d = self.fig_rad.add_subplot(2, 2, 2, projection='3d')
        self.ax_eplane = self.fig_rad.add_subplot(2, 2, 3)
        self.ax_hplane = self.fig_rad.add_subplot(2, 2, 4)
        
        # Style the axes
        for ax in [self.ax_2d, self.ax_eplane, self.ax_hplane]:
    ...
    def plot_radiation_patterns(self, frequency):
        ...
        try:
            ...
            th, gth = self._get_gain_cut(frequency, cut="theta", fixed_angle_deg=0.0)
            ph, gph = self._get_gain_cut(frequency, cut="phi", fixed_angle_deg=90.0)
            ...
            if th is not None and gth is not None:
                ...
                self.ax_2d.plot(theta_rad, gth, 'b-', linewidth=2)
                self.ax_2d.set_title("2D Radiation Pattern")
                self.ax_2d.grid(True)
                
                # Plot E-plane cut
                self.ax_eplane.plot(th, gth, 'b-', linewidth=2, label='E-plane')
                self.ax_eplane.set_xlabel("Angle (degrees)")
                self.ax_eplane.set_ylabel("Gain (dBi)")
                self.ax_eplane.set_title("E-Plane Cut")
            ...
            if ph is not None and gph is not None:
                # Plot H-plane cut
                self.ax_hplane.plot(ph, gph, 'r-', linewidth=2, label='H-plane')
                self.ax_hplane.set_xlabel("Angle (degrees)")
                self.ax_hplane.set_ylabel("Gain (dBi)")
                self.ax_hplane.set_title("H-Plane Cut")
            ...
            # Create a simple 3D radiation pattern (placeholder)
            # In a real implementation, you would get 3D pattern data from HFSS
            theta = np.linspace(0, 2*np.pi, 36)
            phi = np.linspace(0, np.pi, 18)
            theta, phi = np.meshgrid(theta, phi)
            
            # Simple donut-shaped pattern for demonstration
            r = 5 * np.sin(phi) + 10  # Create a donut shape
            ...
            self.ax_3d.plot_surface(x, y, z, cmap='viridis', alpha=0.8)
            self.ax_3d.set_title("3D Radiation Pattern")
    ...
    # Note: Due to character limit, I've included the key organizational changes.
    # The remaining methods (log utilities, physics calculations, AEDT helpers, etc.)
