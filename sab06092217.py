...
class ModernPatchAntennaDesigner:
    ...
    def setup_results_tab(self):
        ...
        # Create figure with 3 subplots: S11, theta cut, phi cut
    ...
    def analyze_and_optimize(self):
        ...
        try:
            ...
            if error_percent < 2:  # Less than 2% error
                messagebox.showinfo("Frequency Analysis", 
                                   f"Resonant frequency ({resonant_freq:.3f} GHz) is within {error_percent:.1f}% of target ({target_freq:.3f} GHz).\nNo optimization needed.")
                return
                
            # Ask user if they want to optimize
            response = messagebox.askyesno("Frequency Optimization", 
                                          f"Resonant frequency ({resonant_freq:.3f} GHz) differs from target ({target_freq:.3f} GHz) by {error_percent:.1f}%.\n"
            ...
            # Calculate scaling factor based on wavelength ratio
            scaling_factor = target_freq / resonant_freq
    ...
    def plot_results(self):
        ...
        try:
            ...
            if sol:
                try:
                    ...
                    if y.size == freqs.size:
                        ...
                        # Plot current data
                        self.ax_s11.plot(freqs, y, label='Current' if self.optimized else 'Simulated', linewidth=2)
            ...
            if th is not None and gth is not None and len(th) == len(gth) and len(th) > 0:
                ...
                # Plot current data
                self.ax_th.plot(th, gth, label='Current' if self.optimized else 'Simulated', linewidth=2)
            ...
            if ph is not None and gph is not None and len(ph) == len(gph) and len(ph) > 0:
                ...
                # Plot current data
                self.ax_ph.plot(ph, gph, label='Current' if self.optimized else 'Simulated', linewidth=2)
