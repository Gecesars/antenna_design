# -*- coding: utf-8 -*-
"""
HFSS Patch Array — λ/4 Transformer + Port Sheet XZ (robusto: sheet por NOME + int line)
...
def _assign_lumped_port_sheet(hfss: Hfss, sheet_name: str, gnd_name: str,
    ...
    """Tenta API de alto nível do PyAEDT; se falhar, faz fallback gRPC com payload completo."""
    int_line = [[_mm(x_mm), _mm(y_mm), _mm(z1_mm)],
                [_mm(x_mm), _mm(y_mm), _mm(z2_mm)]]

    # 1) Tentativa: alto nível (PyAEDT monta 'Objects:=' e 'ReferenceConductors:=' para a gente)
    try:
        ok = hfss.lumped_port(
            assignment=sheet_name,           # *** sheet por NOME ***
            reference=gnd_name,              # ReferenceConductors
            impedance=z0_ohm,
            renormalize=True,
            name=pname,
        ...
        print(f"[WARN] lumped_port (high-level) falhou: {e}")

    # 2) Fallback: payload direto via BoundarySetup.AssignLumpedPort
    try:
        obnd = hfss.odesign.GetModule("BoundarySetup")
        obnd.AssignLumpedPort(
            [
                "NAME:" + pname,
                "Objects:=", [sheet_name],            # sheet por nome!
                "DoDeembed:=", False,
                ...
                "Modes:=", [
                    ...
                    [
                        ...
                        "IntLine:=",
                        [
                            "Start:=", ["X:=", _mm(x_mm), "Y:=", _mm(y_mm), "Z:=", _mm(z1_mm)],
                            "End:=",   ["X:=", _mm(x_mm), "Y:=", _mm(y_mm), "Z:=", _mm(z2_mm)]
                        ],
                        "AlignmentGroup:=", 0,
                        "CharImp:=", "Zpi",
                ...
                "ReferenceConductors:=", [gnd_name],  # *** referência explícita ***
            ]
        )
    except Exception as e:
        raise RuntimeError(f"AssignLumpedPort (fallback) falhou para {pname} no sheet {sheet_name}: {e}")
...
def build_array_project(
    ...
    z_top = "tCu+h"  # topo do dielétrico
    ...
    for ix in range(nx):
        for iy in range(ny):
            ...
            patch_y_min_val = cy - Lp/2.0
            line_y0_val = patch_y_min_val - Lq
            feed_y0_val = line_y0_val - Pad
            ...
            # --------- PORT SHEET (XZ) ----------
            # Sheet em XZ no y do feed, atravessando cobre inf + h + cobre sup
            sheet_name = f"PORTSHEET_{ix+1}_{iy+1}"
            hfss.modeler.create_rectangle(
                origin=[f"{cx_s}-Wt/2", f"{feed_y0_val:.6f}mm", "0"],
                sizes=["Wt", f"{h_mm + 2*COPPER_T:.6f}mm"],
                orientation="XZ",
                name=sheet_name
            )

            # --------- PORTA LUMPED ----------
            # Integração em Z, no mesmo (x,y) do sheet (plano XZ)
            z1 = 0.5 * COPPER_T
            z2 = h_mm + 1.5 * COPPER_T
...
class App(ctk.CTk):
    def __init__(self):
        ...
        self._log("[Fix] Porta lumped: sheet por NOME + ReferenceConductors + Integration Line (Z).")
