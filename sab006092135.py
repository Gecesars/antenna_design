...
class ModernPatchAntennaDesigner:
    def __init__(self):
        ...
        # última análise de S11 (resultados para usar na correção)
        self.last_s11_analysis = None  # dict: {"f": array, "s11_db": array, "f_res": float, "s11_min_db": float, "R":float|None, "X":float|None, "port_expr": "(P1_Lumped,P1_Lumped)"}

        # -------- Parâmetros do usuário (iniciais) --------
        self.params = {
            "frequency": 10.0,             # GHz
            "gain": 12.0,                  # dBi (apenas sizing simples)
            "sweep_start": 8.0,            # GHz
            "sweep_stop": 12.0,            # GHz
            "cores": 4,
            "aedt_version": "2024.2",
            ...
            "feed_rel_x": 0.485,           # 0..1 ao longo de W
    ...
    def _build_results_tab(self):
        ...
        ctk.CTkLabel(main, text="S11 Analysis & Tuning", font=ctk.CTkFont(size=16, weight="bold")).grid(row=0, column=0,
                                                                                                         pady=10)
        ...
        # 1 eixo: S11
        self.fig = plt.figure(figsize=(9, 5.8))
        ...
        ctrl.grid(row=2, column=0, pady=8)
        ...
        ctk.CTkButton(ctrl, text="Apply Correction", command=self.apply_frequency_correction,
                      fg_color="#20B2AA", hover_color="#40E0D0").pack(side="left", padx=8)

        self.result_label = ctk.CTkLabel(main, text="",
                                         font=ctk.CTkFont(weight="bold"))
        self.result_label.grid(row=3, column=0, pady=(6, 2))

        exp = ctk.CTkFrame(main)
        exp.grid(row=4, column=0, pady=8)
    ...
    def export_csv(self):
        try:
            if self.last_s11_analysis:
                ...
                data = np.column_stack((f, s11))
                np.savetxt("s11_analysis.csv", data, delimiter=",", header="Frequency (GHz), S11 (dB)", comments='')
                self.log_message("Data exported to s11_analysis.csv")
            else:
    ...
    def export_png(self):
        try:
            if hasattr(self, 'fig'):
                self.fig.savefig("results_s11.png", dpi=300, bbox_inches='tight')
                self.log_message("Plot saved to results_s11.png")
        except Exception as e:
    ...
    def _create_coax_feed_lumped(self, ground, substrate, x_feed: float, y_feed: float, name_prefix: str):
        try:
            ...
            self.log_message(f"Lumped Port '{name_prefix}_Lumped:1' created (integration line).")
    ...
    def _add_post_var(self, name: str, value: str) -> bool:
        """Cria variável de pós-processamento local (PostProcessingVariableProp)."""
        try:
            props = [
                "NAME:AllTabs",
                [
    ...
    def _sorted_excitations(self) -> List[str]:
        """Retorna ['P1_Lumped:1', 'P2_Lumped:1', ...] ordenado por índice numérico."""
        try:
    ...
    def _edit_sources_with_vars(self, excitations: List[str], pvars: List[str], phvars: List[str]) -> bool:
        """Aplica Solutions.EditSources usando p_i e ph_i."""
        try:
    ...
    def _create_infinite_sphere_after(self, name="Infinite Sphere1"):
        """Cria a esfera infinita via módulo RadField (θ/φ -180..180, 1°)."""
        try:
    ...
    def _postprocess_after_solve(self):
        """Após a solução: criar p_i/ph_i, aplicar EditSources, criar esfera infinita."""
        try:
            exs = self._sorted_excitations()
            if not exs:
            ...
            for i in range(1, len(exs) + 1):
                p = f"p{i}"
                ph = f"ph{i}"
                self._add_post_var(p, "1W")
                self._add_post_var(ph, "0deg")
                pvars.append(p)
                phvars.append(ph)
    ...
    def _fetch_solution(self, expression: str):
        """
        Cria um relatório padrão (Standard) e devolve o objeto de solução para uma expressão.
        Tenta 'Setup1 : Sweep1' (domínio Sweep). Se falhar, tenta 'Setup1 : LastAdaptive'.
        """
        try:
            rpt = self.hfss.post.reports_by_category.standard(
            ...
            self.log_message(f"fetch_solution primary failed for '{expression}': {e}")

        # Fallback: LastAdaptive
        try:
            rpt = self.hfss.post.reports_by_category.standard(
                expressions=[expression],
            ...
            self.log_message(f"fetch_solution fallback failed for '{expression}': {e}")
    ...
    def _shape_series(self, data_obj, npoints: int) -> np.ndarray:
        """
        Converte a saída de sol.data_*() em array 1-D (npoints).
        Aceita formatos: [[...]], [...], valor escalar.
        """
        ...
        if arr.size == 1 and npoints > 1:
            return np.array([], dtype=float)
        if arr.size != npoints and npoints > 0:
            arr = np.ravel(arr)
            if arr.size != npoints:
                return np.array([], dtype=float)
        return arr
    ...
    def _get_s11_curves(self, port_expr: str):
        """Retorna (f_ghz, s11_db, reS, imS) para port_expr='(P1_Lumped,P1_Lumped)'."""
        sol_db, _ = self._fetch_solution(f"dB(S{port_expr})")
        if not sol_db:
            return None
        try:
            f_db = np.asarray(sol_db.primary_sweep_values, dtype=float)
            y_db_raw = sol_db.data_real()
            y_db = self._shape_series(y_db_raw, f_db.size)
            if y_db.size == 0:
                # fallback: usa |S| e converte pra dB
                sol_mag, _ = self._fetch_solution(f"mag(S{port_expr})")
                if not sol_mag:
                    return None
                f_db = np.asarray(sol_mag.primary_sweep_values, dtype=float)
                mag = self._shape_series(sol_mag.data_real(), f_db.size)
                if mag.size == 0:
                    return None
                y_db = 20.0 * np.log10(np.clip(mag, 1e-16, None))
        except Exception:
            return None

        # Re/Im (opcional, para impedância)
        reS = imS = None
        sol_re, _ = self._fetch_solution(f"re(S{port_expr})")
        sol_im, _ = self._fetch_solution(f"im(S{port_expr})")
        if sol_re and sol_im:
            try:
                f_re = np.asarray(sol_re.primary_sweep_values, dtype=float)
                f_im = np.asarray(sol_im.primary_sweep_values, dtype=float)
                if f_re.size == f_db.size == f_im.size and np.allclose(f_re, f_db) and np.allclose(f_im, f_db):
                    reS = self._shape_series(sol_re.data_real(), f_db.size)
                    imS = self._shape_series(sol_im.data_real(), f_db.size)
                    if reS.size == 0 or imS.size == 0:
                        reS = imS = None
            except Exception:
                reS = imS = None
    ...
    def run_simulation(self):
        try:
            ...
            try:
                ...
                exs = []
            self.log_message(f"Excitations created: {len(exs)} -> {exs if exs else []}")
            ...
            # >>> Pós-processamento após a solução (como solicitado)
    ...
    def analyze_and_mark_s11(self):
        """Cria/atualiza o gráfico S11, acha o mínimo e calcula impedância no ponto."""
        try:
            # Escolhe o primeiro porto
            exs = self._sorted_excitations()
            if not exs:
                self.log_message("Analyze S11 error: no excitations.")
                return
            p = exs[0].split(":")[0]  # "P1_Lumped"
            port_expr = f"({p},{p})"
            ...
            s11_db = np.atleast_1d(s11_db)
            if f.size == 0 or s11_db.size == 0 or f.size != s11_db.size:
                self.log_message("S11 analysis aborted: empty or mismatched curve.")
                return

            # Plot
            self.ax_s11.clear()
            self.ax_s11.plot(f, s11_db, linewidth=2, label=f"S11 {p}")
            ...
            self.ax_s11.axvline(x=cf, linestyle='--', alpha=0.7, label=f'f0={cf:g} GHz')

            # mínimo
            idx_min = int(np.argmin(s11_db))
            f_res = float(f[idx_min])
            s11_min_db = float(s11_db[idx_min])
            self.ax_s11.scatter([f_res], [s11_min_db], s=50, marker="o")
            ...
            # impedância
            R = X = None
            if reS is not None and imS is not None and reS.size == f.size and imS.size == f.size:
                S = complex(float(reS[idx_min]), float(imS[idx_min]))
                Z0 = 50.0
                try:
                    Z = Z0 * (1 + S) / (1 - S)
                    R = float(np.real(Z))
            ...
            # guardar análise
            self.last_s11_analysis = {
                "f": f, "s11_db": s11_db, "f_res": f_res, "s11_min_db": s11_min_db,
                "R": R, "X": X, "port_expr": port_expr
            }

            # UI resumo
            if R is not None and X is not None:
                self.result_label.configure(
                    text=f"Min @ {f_res:.4g} GHz, S11={s11_min_db:.2f} dB, Z≈{R:.1f} + j{X:.1f} Ω"
                )
            else:
                self.result_label.configure(
                    text=f"Min @ {f_res:.4g} GHz, S11={s11_min_db:.2f} dB (Z unavailable)"
                )
            self.log_message("S11 analyzed and plotted.")
        except Exception as e:
            self.log_message(f"Analyze S11 error: {e}")
            self.log_message(f"Traceback: {traceback.format_exc()}")

    def apply_frequency_correction(self):
        """Aplica correção de frequência (regra de 3 inversa) às dimensões e atualiza variáveis no AEDT."""
        try:
            ...
            f_res = float(self.last_s11_analysis["f_res"])
            if f_res <= 0 or f_target <= 0:
                self.log_message("Invalid frequencies for correction.")
                return

            # regra de 3 inversa: dimensões ∝ 1/f → escala = f_res / f_target
            scale = f_res / f_target

            # atualiza parâmetros calculados
            self.calculated_params["patch_length"] *= scale
            self.calculated_params["patch_width"]  *= scale
            self.calculated_params["spacing"]      *= scale

            # recomputa substrato (com base nas novas dimensões)
            self.calculate_substrate_size()

            # escreve de volta no AEDT (variáveis paramétricas)
            self.hfss["patchL"] = f"{self.calculated_params['patch_length']:.6f}mm"
            self.hfss["patchW"] = f"{self.calculated_params['patch_width']:.6f}mm"
            self.hfss["spacing"] = f"{self.calculated_params['spacing']:.6f}mm"
            self.hfss["subW"] = f"{self.calculated_params['substrate_width']:.6f}mm"
            self.hfss["subL"] = f"{self.calculated_params['substrate_length']:.6f}mm"
            ...
            self.log_message(f"Applied frequency correction: scale={scale:.6f} → dimensions {sense}.")
            self.result_label.configure(
                text=f"Applied correction (scale={scale:.6f}). Re-run simulation to verify."
            )
        except Exception as e:
